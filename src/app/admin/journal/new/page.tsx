...
export default function NewJournalPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      addToast("success", "Journal entry created successfully");
      router.push("/admin/journal");
    } catch (err) {
      addToast("error", err instanceof Error ? err.message : "Failed to create journal entry");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <h1 className="text-2xl font-bold text-primary mb-8">New Journal Post</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-6 max-w-3xl">
        <section className="bg-surface rounded-xl border border-border p-6 space-y-4">
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            ...
            />
            <span className="text-sm text-primary">Publish immediately</span>
          </label>
        </section>
